      ...
      {refundRequests.length > 0 ? (
        <div className="space-y-4">
          {refundRequests.map((request) => (
            <div key={request.product_id} className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
              <div className="grid grid-cols-6 gap-4 items-center">
                ...
                <div>
                  <button
                    onClick={() =>
                      setExpandedRequest(expandedRequest === request.product_id ? null : request.product_id)
                    }
                  ...
                  >
                    Details {expandedRequest === request.product_id ? <ChevronUp size={16} className="ml-1" /> : <ChevronDown size={16} className="ml-1" />}
                  </button>
                </div>
